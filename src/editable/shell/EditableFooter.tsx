  ...
  return (
    <footer className="mt-auto bg-[var(--editable-footer-bg)] text-[var(--editable-footer-text)]">
      <div className="mx-auto max-w-[var(--editable-container)] px-4 py-14 sm:px-6 lg:px-8">
        <div className="grid gap-10 rounded-[2rem] border border-[rgba(255,219,253,0.16)] bg-[rgba(99,103,255,0.18)] px-6 py-8 sm:px-8 lg:grid-cols-[1.2fr_0.9fr_0.9fr_1fr]">
          <div>
            ...
            <p className="mt-5 max-w-md text-sm leading-7 text-[rgba(255,219,253,0.78)]">
              {globalContent.footer?.description || SITE_CONFIG.description}
            </p>
            <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-[rgba(255,219,253,0.16)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-[rgba(255,219,253,0.72)]">
          ...
          <div>
            <div className="grid gap-2.5">
              {taskLinks.slice(0, 6).map((task) => (
                <Link key={task.key} href={task.route} className="inline-flex items-center gap-2 text-sm text-[rgba(255,219,253,0.78)] transition hover:text-[var(--editable-footer-text)]">
          ...
          <div>
            ...
            <div className="mt-5 grid gap-2.5">
              {[
                ...
                <Link key={href} href={href} className="text-sm text-[rgba(255,219,253,0.78)] transition hover:text-[var(--editable-footer-text)]">
                  {label}
                </Link>
              ))}
              {session ? (
                <button type="button" onClick={logout} className="text-left text-sm text-[rgba(255,219,253,0.78)] transition hover:text-[var(--editable-footer-text)]">
                  Logout
                </button>
              ) : null}
            </div>
          ...
          <div>
            ...
            <div className="mt-5 space-y-3 text-sm leading-7 text-[rgba(255,219,253,0.78)]">
        ...
        <div className="mt-6 flex flex-col gap-3 border-t border-[rgba(255,219,253,0.16)] pt-6 text-sm text-[rgba(255,219,253,0.58)] sm:flex-row sm:items-center sm:justify-between">
