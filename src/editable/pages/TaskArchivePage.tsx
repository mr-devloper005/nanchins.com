...
const taskGrid: Record<TaskKey, string> = {
  article: 'grid gap-6 lg:grid-cols-2 2xl:grid-cols-3',
...
const archiveAds: Record<TaskKey, { top: 'header' | 'in-feed' | 'sidebar' | 'footer'; bottom: 'header' | 'in-feed' | 'sidebar' | 'footer' }> = {
  ...
  pdf: { top: 'header', bottom: 'footer' },
  profile: { top: 'sidebar', bottom: 'in-feed' },
}
      ...
      <main style={taskThemeStyle(task)} className="min-h-screen bg-[var(--tk-bg)] text-[var(--tk-text)]">
        <header className="relative overflow-hidden border-b border-[var(--tk-line)]">
          ...
          <div className="relative mx-auto max-w-[var(--editable-container)] px-6 py-20 sm:py-24 lg:px-8">
            ...
            <h1 className="editable-display mt-6 max-w-4xl text-balance text-[2.8rem] font-semibold leading-[1.02] tracking-[-0.04em] sm:text-5xl lg:text-[4.2rem]">
            ...
            {voice?.chips?.length ? (
              <div className="mt-8 flex flex-wrap gap-2.5">
                {voice.chips.map((chip) => (
                  <span key={chip} className="rounded-full border border-[var(--tk-line)] bg-[var(--tk-surface)] px-3.5 py-1.5 text-xs font-medium text-[var(--tk-muted)]">
                    {chip}
                  </span>
                ))}
              </div>
            ...
            <div className="mt-12 flex flex-col gap-4 rounded-[1.6rem] border border-[var(--tk-line)] bg-[color:var(--tk-surface)]/70 p-4 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
              ...
              <form action={basePath} className="flex flex-col gap-2.5 sm:flex-row sm:items-center">
                <div className="relative min-w-[220px]">
                  <select
                    name="category"
                    defaultValue={category}
                    className="h-12 w-full appearance-none rounded-full border border-[var(--tk-line)] bg-[var(--tk-bg)]/30 pl-4 pr-10 text-sm font-medium text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
                ...
                <button className="inline-flex h-12 items-center justify-center text-sm font-semibold uppercase tracking-[0.16em] text-[var(--tk-accent)] transition hover:underline">
                  Apply
                </button>
              </form>
            </div>
          ...
          {posts.length ? (
            <nav className="mt-16 flex flex-wrap items-center justify-center gap-3 text-sm">
              {pagination.hasPrevPage ? (
                <Link href={pageHref(basePath, category, page - 1)} className="font-medium text-[var(--tk-accent)] transition hover:underline">
                  Previous
                </Link>
              ) : null}
              <span className="font-medium text-[var(--tk-muted)]">
...
function ListingArchiveCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link href={href} className={`${cardBase} flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:p-6`}>
      ...
      <ArrowUpRight className="h-5 w-5 shrink-0 self-start text-[var(--tk-muted)] transition group-hover:text-[var(--tk-accent)] sm:self-center" />
    </Link>
  )
}
