...
export function EditableArticleComments({ slug, comments = [] }: { slug: string; comments?: Comment[] }) {
  ...
  const persist = (next: Comment[]) => {
    ...
    try {
      ...
      /* ignore storage issues and keep in-memory comments */
    }
  }
    ...
    <section className="mt-14 border-t border-[var(--tk-line)] pt-10">
      ...
      <form onSubmit={submit} className="mt-6 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 shadow-[0_18px_34px_rgba(183,4,4,0.18)]">
        <input
          ...
          maxLength={60}
          className="h-12 w-full rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-bg)]/40 px-4 text-sm text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <textarea
          value={text}
          ...
          rows={3}
          maxLength={1500}
          className="mt-3 w-full resize-y rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-bg)]/40 px-4 py-3 text-sm leading-6 text-[var(--tk-text)] outline-none transition focus:border-[var(--tk-accent)]"
        />
        <div className="mt-3 flex justify-end">
          <button
            type="submit"
            disabled={!text.trim()}
            className="inline-flex items-center gap-2 rounded-full bg-[var(--tk-accent)] px-6 py-3 text-sm font-bold text-[var(--tk-on-accent)] transition hover:brightness-95 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
          >
